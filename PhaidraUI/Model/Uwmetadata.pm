package PhaidraUI::Model::Uwmetadata;
...
sub save_uwmetadata(){
	...
	my $soapres = $soap->modifyDatastreamByValue($pid, "UWMETADATA", '', $c->app->config->{phaidra}->{uwmetadatalabel}, "text/xml", '', SOAP::Data->type(base64 => $uwmetadata), 'DISABLED', 'none', 'Modified by PhaidraUI (rest)', SOAP::Data->type(boolean => 0));
	if($soapres->fault)
	{
