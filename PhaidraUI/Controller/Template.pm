...
sub load {
	...
	#}
	if($doc->{uwmetadata}){
		$self->app->log->info("load template uwmetadata");
	...
	if($doc->{mods}){
		$self->app->log->info("load template mods");
		...
		my @templateClassifications;
		foreach my $n (@{$doc->{mods}}){
		     if($n->{xmlname} eq "classification"){
		         push(@templateClassifications, $n);  
		     }
		}
		...
		$self->render(
			json => {
				...
				title => $doc->{title},
				classification => \@templateClassifications, 
			},
			status => 200
		);
		return;
