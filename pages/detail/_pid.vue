    ...
    <template v-if="objectInfo">
      ...
      <v-row v-if="objectInfo.cmodel === 'Page'" justify="center">
        <v-col cols="6">
          ...
          <v-row justify="center" class="mt-4"><v-btn large raised color="primary" :to="localePath({ path: `/detail/${objectInfo.bookpid}`})">{{ $t('Go to book') }}</v-btn></v-row>
        </v-col>
      </v-row>
      <v-row v-else-if="objectInfo.ismemberof && (objectInfo.ismemberof.length > 0)" justify="center">
        <template v-if="objectInfo.ismemberof.length === 1">
          <v-col cols="6">
            ...
            <v-row justify="center" class="mt-4"><v-btn large raised color="primary" :to="localePath({ path: `/detail/${objectInfo.ismemberof[0]}`})">{{ $t('Go to container') }}</v-btn></v-row>
          </v-col>
        </template>
        <template v-else>
          <v-col cols="6">
            <v-row justify="center" class="mt-5">{{ $t('This object is a member of multiple containers. Please choose a container to view:') }}</v-row>
            <v-row justify="center" class="mt-4"><v-btn v-for="(contpid, i) in objectInfo.ismemberof" :key="'contbtn'+i" large raised color="primary" :to="localePath({ path: `/detail/${contpid}`})">{{ $t('Go to container') }}&nbsp;{{ contpid }}</v-btn></v-row>
          </v-col>
        </template>
      </v-row>
      <v-row v-else>
          ...
          <v-row justify="end">
                  ...
                  <v-card tile>
                    ...
                    <v-card-text class="mt-4">
                      <v-row no-gutters class="pt-2" justify="start">
                        ...
                        <v-btn v-if="objectInfo.cmodel === 'Collection'" :to="localePath({ path: '/search', query: { collection: objectInfo.pid, reset: 1 }})" color="primary">{{ $t('Show members') }} ({{ objectInfo.haspartsize }})</v-btn>
            ...
            <client-only>
              ...
              <v-row class="my-6">
                <v-col class="pt-0">
                  <v-card tile>
                    <v-card-title class="ph-box title font-weight-light grey white--text">
                      <nuxt-link class="white--text" :to="localePath(`/stats/${objectInfo.pid}`)">
                    ...
                    <v-card-text class="mt-4">
                      <template v-for="(rel, i) in objectInfo.versions">
                        <v-row :key="'version'+i">
                          ...
                          <v-col cols="12" md="7">
                            <nuxt-link :to="localePath(`/detail/${rel.pid}`)">{{ rel.dc_title[0] }}</nuxt-link>
                          </v-col>
                        </v-row>
                  ...
                  <v-card tile>
                    ...
                    <v-card-text class="mt-4">
                      <v-row no-gutters class="pt-2" v-if="objectInfo.dshash['JSON-LD']">
                        <nuxt-link :to="localePath(`/metadata/${objectInfo.pid}`)">{{ $t('Show metadata') }}</nuxt-link>
                      </v-row>
              ...
              <v-row class="my-6" v-if="objectInfo.writerights === 1">
                <v-col class="pt-0">
                  <v-card tile>
                    ...
                    <v-card-text class="mt-4">
                      <v-row no-gutters class="pt-2" v-if="objectInfo.dshash['JSON-LD']">
                        <nuxt-link :to="localePath('metadataeditor')">{{ $t('Edit metadata') }}</nuxt-link>
                      </v-row>
                      <v-row no-gutters class="pt-2" v-if="objectInfo.dshash['UWMETADATA']">
                        <nuxt-link :to="localePath('uwmetadataeditor')">{{ $t('Edit metadata') }}</nuxt-link>
                      </v-row>
                      <v-row no-gutters class="pt-2" v-if="(objectInfo.cmodel === 'Container') || (objectInfo.cmodel === 'Collection')">
                        <nuxt-link class="mb-1" :to="localePath('sort')">{{ $t('Sort members') }}</nuxt-link>
                      </v-row>
                      <v-row no-gutters class="pt-2" v-if="objectInfo.cmodel === 'Container'">
                        <nuxt-link class="mb-1" :to="localePath('addmember')">{{ $t('Upload member') }}</nuxt-link>
                      </v-row>
                      <v-row no-gutters class="pt-2" v-if="(objectInfo.cmodel !== 'Container') && (objectInfo.cmodel !== 'Collection') && (objectInfo.cmodel !== 'Resource') && (objectInfo.cmodel !== 'Book') && (objectInfo.cmodel !== 'Page')">
                        <nuxt-link class="mb-1" :to="localePath(`upload-webversion/${objectInfo.pid}`)">{{ $t('Upload web-optimized version') }}</nuxt-link>
                      </v-row>
                        ...
                        <v-dialog class="pb-4" v-model="relationDialog" width="800px">
                          ...
                          <v-card>
                            ...
                            <v-card-actions>
                              ...
                              <v-btn class="primary" :disabled="!chosenRelation" @click="$router.push(localeLocation({ name: 'submit-related', params: { relatedpid: objectInfo.pid, relation: chosenRelation }}))">{{ $t('Continue') }}</v-btn>
                            </v-card-actions>
                          </v-card>
                        </v-dialog>
                      </v-row>
                      <v-row no-gutters class="pt-2" v-if="(objectInfo.cmodel !== 'Container') && (objectInfo.cmodel !== 'Collection')">
                        <nuxt-link class="mb-1" :to="localePath('rights')">{{ $t('Access rights') }}</nuxt-link>
                      </v-row>
                      <v-row no-gutters class="pt-2">
                        <nuxt-link class="mb-1" :to="localePath('relationships')">{{ $t('Relationships') }}</nuxt-link>
                      </v-row>
                      <v-row no-gutters class="pt-2">
                        <nuxt-link class="mb-1" :to="localePath('delete')">{{ $t('Delete') }}</nuxt-link>
                      </v-row>
                    </v-card-text>
                  </v-card>
                </v-col>
