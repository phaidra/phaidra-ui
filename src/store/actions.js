...
export default {
  ...
  async login ({ commit, dispatch, state }, credentials) {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        if (process.browser) {
          document.cookie = 'X-XSRF-TOKEN=' + response.data['XSRF-TOKEN'] + '; domain=' + state.instanceconfig.baseurl + '; secure;'
        }
  ...
  async logout ({ commit, dispatch, state }) {
    if (process.browser) {
      document.cookie = 'X-XSRF-TOKEN=;expires=Thu, 01 Jan 1970 00:00:01 GMT'
    }
    try {
